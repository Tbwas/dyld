...
static const struct mach_header* randomizeExecutableLoadAddress(const struct mach_header* orgMH, uintptr_t* appsSlide)
{
	...
	uintptr_t lowestAddressUsed = UINTPTR_MAX; // 0xffff ffff ffff ffff
	cmd = cmds;
	for (uint32_t i = 0; i < cmd_count; ++i) {
		if ( cmd->cmd == LC_SEGMENT_COMMAND ) {
			const struct macho_segment_command* segCmd = (struct macho_segment_command*)cmd;
			if ( (strcmp(segCmd->segname, "__PAGEZERO") != 0) && (strcmp(segCmd->segname, "__UNIXSTACK") != 0) ) {
				...
				if ( (segCmd->vmaddr + segCmd->vmsize) > highestAddressUsed )
					highestAddressUsed = ((segCmd->vmaddr + segCmd->vmsize) + 4095) & -4096; // -4096 = 0xffff ffff ffff f000
...
//    char *base = ((char*)&_mh_dylinker_header);
    char *base = 0x0;
	const struct macho_header* dyldsMachHeader = (const struct macho_header*)(base+slide);
